...
from fieldscale.python.fieldscale import Fieldscale
...
class ThermalImuSync:
    ...
    def thermal_callback(self, msg):
        ...
        # rescaled = fieldscale(img) / 2**16
        # rescaled = fieldscale(img) /  2**8
        # rescaled = fieldscale(img) 
        # rescaled = rescaled.astype(np.float32) / 255.0
        ...
        # print(" equalized_img  is " , equalized_img.astype(np.float32))
        # print(" rescaled  is " , rescaled.astype(np.float32))
        
        # return img, rescaled.astype(np.float32)
        # return img, rescaled
